<?php
	...
	if($_SERVER["REQUEST_METHOD"]=="POST")
		...
		if(!empty($fname) && !empty($lname) && !empty($email) && !empty($contact) && !empty($pswd) && !empty($schno))
		{
			...
			echo "<a href='index1.php'>Click here</a> to Login<br>";
		}
		else
		{
		...
		$checkno = "SELECT * FROM student where mobno = '$contact'";
		...
		{
			$sql2 = "INSERT INTO student (fname,lname,email,mobno,schno,pswd)
